...
function loadPlugins() {
  ...
  // ── Set up global.checkAccess — reads mode dynamically from settings.json ──
  try {
    const ac = require("./plugins/accesscontrol");
    global.checkAccess = function(sessionId, senderNumber, isOwner, isGroup) {
      // Always read fresh from settings.json so .setmode takes effect instantly
      const mode = (ac.getSetting ? ac.getSetting("mode") : null) || (config.MODE || "public");

      // Check ban list first
      if (ac.isBanned && ac.isBanned(senderNumber + "@s.whatsapp.net")) {
      ...
      switch (mode.toLowerCase()) {
        case "private":
          return { allowed: false, reason: "🔒 Bot is in *Private* mode. Only owner can use commands." };
        case "inbox":
          if (isGroup) return { allowed: false, reason: "📩 Bot is in *Inbox* mode. Use commands in private chat only." };
          return { allowed: true };
        case "group":
          if (!isGroup) return { allowed: false, reason: "👥 Bot is in *Group* mode. Use commands in a group only." };
          return { allowed: true };
        case "premium":
          if (ac.isPremium && ac.isPremium(senderNumber + "@s.whatsapp.net")) return { allowed: true };
          return { allowed: false, reason: "💎 Bot is in *Premium* mode. Only premium users can use commands." };
        case "privatepremium":
          if (isGroup) return { allowed: false, reason: "💎 Bot is in *Private Premium* mode. Use in private chat only." };
          if (ac.isPremium && ac.isPremium(senderNumber + "@s.whatsapp.net")) return { allowed: true };
          return { allowed: false, reason: "💎 Bot is in *Private Premium* mode. Only premium users in private chat." };
        case "public":
        default:
          return { allowed: true };
      }
    };
    console.log("✅ global.checkAccess registered with full mode support");
  } catch(e) {
    console.error("[checkAccess setup failed]:", e.message);
    global.checkAccess = () => ({ allowed: true });
  }
