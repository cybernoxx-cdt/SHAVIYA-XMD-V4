...
function loadPlugins() {
  ...
  // ── Set up global.checkAccess using accesscontrol plugin ──
  try {
    ...
    global.checkAccess = function(sessionId, senderNumber, isOwner, isGroup) {
      ...
      if (mode === "private" && !isOwner) {
        return { allowed: false, reason: "❌ This bot is in *private* mode. Only the owner can use commands." };
      }
      return { allowed: true };
    };
    console.log("✅ global.checkAccess registered (mode: " + (config.MODE || "public") + ")");
  } catch(e) {
